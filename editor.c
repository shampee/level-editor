...
typedef struct EDITOR_CONTROL
{
    ...
    short   mapX, mapY, 
            ...
            mButton, 
            setIndex, pieceIndex, shortcutIndex, 
            grid;
...
typedef struct LevelInfo
{
    short width, height, 
    width_z, height_z,
    tile_size, tile_size_z, 
    total_tiles, tile_pieces, total_tile_pieces,
    tile_piece_size, tile_piece_size_z;
} Level;
...
void startInputs(Editor *e, SDL_Event event, Button buttons[], const size_t size, L_File *list);
void editInputs(Editor *e, SDL_Event event, Level l);
void loadInputs(Editor *e, SDL_Event event, Button buttons[], L_File *file_list, Level level);
void newInputs(Editor *e, SDL_Event event, Button buttons[], S_Input *input, Level *l);
void menuInputs(Editor *e, SDL_Event event, Button buttons[], const size_t bsize);
...
void startRender(Editor *e, SDL_Renderer *renderer, FC_Font *texture, Button buttons[]);
void editRender(Editor *e, SDL_Renderer *renderer, texture texture, FC_Font *f, Level level);
void loadRender(Editor *e, SDL_Renderer *r, FC_Font *t, Button btns[], SDL_Rect fload, L_File list[]);
void menuRender(Editor *e, SDL_Renderer *r, FC_Font *t, Button btns[]);
void newRender(Editor *e, SDL_Renderer *r, FC_Font *t, Button btns[], S_Input input);

void freeTexture(texture *text);
void renderTexture(texture *text, int x, int y, SDL_Rect *clip, const SDL_RendererFlip flip, bool zoom);

SDL_Texture *loadTexture(char path[16]); 

void renderTiles(texture *sheet, Editor editor, Level level);
void renderTileTexture(texture *sheet, Editor editor, int i, int x, int y, int x2, int y2);
...
void renderGrid(SDL_Renderer *r, SDL_Rect *c, short draw, bool zoom, Level level);
void renderShortcuts(SDL_Renderer *r, texture *sheet, hudTile hudTile[], SDL_Rect *clips);
...
int main() 
{
    ...
    if (initSdl())
    {
        ...
        FC_LoadFont(fontTexture, renderer, "ASCII.ttf", 16, FC_MakeColor(0, 0, 0, 255), TTF_STYLE_NORMAL);
        ...
        while (!editor->quit)
        {
            ...
            switch (editor->state)
            {
                ...
                break;
                case E_LOAD: 
                    loadRender(editor, renderer, fontTexture, buttons, fileLoader, file_list);
                break;
            }
...
bool initTextureMap(texture *sheet, char *str)
{
    ...
    if (sheet->mTexture == NULL) success = false;
    else 
    {
        sheet->mWidth = 272;
        sheet->mHeight = 128;
...
void startInputs(Editor *editor, SDL_Event e, Button buttons[], const size_t size, L_File *list)
{
        ...
        switch (e.type)
        {
            ...
            case SDL_KEYDOWN:
                switch (e.key.keysym.sym)
                {
                    ...
                    break;
                    case SDLK_LALT:
                        if (editor->grid++ > 1) editor->grid = 0;
                    break;
                    case SDLK_LCTRL:
                        editor->hold = true;
                    break;
            ...
            case SDL_MOUSEMOTION:
                if (!editor->hold)
                {
                    short   lx = editor->zoom ? l.width_z - editor->camera.x : l.width - editor->camera.x, 
                            ly = editor->zoom ? l.height_z - editor->camera.y : l.height - editor->camera.y;
                    ...
                    {
                        editor->selectedBox.w = editor->zoom ? l.tile_size : l.tile_piece_size;
                        editor->selectedBox.h = editor->zoom ? l.tile_size : l.tile_piece_size;

                        // so proud of this lol, trial and error
                        editor->mapX = (e.motion.x + editor->camera.x + l.width - l.width) >> editor->tileScaled;
                        editor->mapY = (e.motion.y + editor->camera.y + l.height - l.height) >> editor->tileScaled;

                        editor->selectedBox.x = (editor->mapX << editor->tileScaled) - editor->camera.x;
                       editor->selectedBox.y = (editor->mapY << editor->tileScaled) - editor->camera.y;

                        editor->pieceIndex = ((editor->mapY >> 1) * 20) + (editor->mapX >> 1);
                        editor->setIndex = ((editor->mapY % 2) * 2) + (editor->mapX % 2);

                        if (editor->pressed)
                        {
                            if (editor->mButton == SDL_BUTTON_LEFT) 
                            {
                                editor->tileSet[editor->pieceIndex].set[editor->setIndex] = editor->hudShortcuts[editor->shortcutIndex].id;
                            }
                            else if (editor->mButton == SDL_BUTTON_RIGHT) 
                            {
                                editor->tileSet[editor->pieceIndex].set[editor->setIndex] = 135;
                            }
                        }
                    }
                }
            ...
            case SDL_MOUSEBUTTONDOWN:
                if (!editor->hold)
                {
                    ...
                    && ((e.motion.y < 128) && (e.motion.y >= 0)))
                    {
                        editor->hudShortcuts[editor->shortcutIndex].id = (editor->tileY * 17) + editor->tileX;
                    }
...
void loadInputs(Editor *edit, SDL_Event event, Button buttons[], L_File *list, Level level)
{
    while (SDL_PollEvent(&event) != 0)
    {
        switch (event.type)
        {
            ...
            case SDL_MOUSEBUTTONDOWN:
                ...
                && event.motion.x <= buttons[B_NEW].box.x + buttons[B_NEW].box.w) 
                && event.motion.y > buttons[B_NEW].box.y 
                && event.motion.y <= buttons[B_NEW].box.y + 130)    // list box size
                {
                    for (int i = 0; i < 20; i++)
                    {
                        ...
                        {
                            if (loadMap(edit->tileSet, edit->fileBuffer, level, list[i].str)) // only for testing !!!
                            {
    ...
    while (SDL_PollEvent(&event) != 0)
    {
        switch (event.type)
        {
            ...
            case SDL_MOUSEMOTION:
                ...
                else buttons[B_EXIT].hover = false;
            break;
            case SDL_MOUSEBUTTONDOWN:
                if ((event.motion.x > buttons[B_LOAD].box.x 
                && event.motion.x <= buttons[B_LOAD].box.x + buttons[B_LOAD].box.w) 
                    ...
                    switch (event.key.keysym.sym)
                    {
                        case SDLK_RETURN:
                            if (!e->create)
                            {
                                ...
                                {
                                    if (loadMap(e->tileSet, e->fileBuffer, *l, e->fileName))
                                        e->state = E_EDIT;
                        ...
                        break;
                        case SDLK_LEFT:
                            if (input->str_cursor > 0) input->str_cursor--;
                        break;
                        case SDLK_RIGHT:
                            if (input->str_cursor < 32 
                            && input->str_cursor < input->str_selection_len) input->str_cursor++;
                        break;
                        case SDLK_BACKSPACE:
                            if (input->str_cursor > 0) 
                            {
                                ...
                                for (int i = input->str_cursor; i <= input->str_selection_len; i++)
                        ...
                        case SDLK_a ... SDLK_z:
                            ...
                            {
                                for (int i = input->str_selection_len; i >= input->str_cursor; i--)
                                    input->string[i] = input->string[i - 1];
...
void menuInputs(Editor *e, SDL_Event event, Button buttons[], const size_t bsize)
{
    while (SDL_PollEvent(&event) != 0)
    {
        switch (event.type)
        {
            ...
            break;
            case SDL_KEYDOWN:
                if (event.key.keysym.sym == SDLK_ESCAPE) e->state = E_EDIT;
            break;
            case SDL_MOUSEMOTION:
                if ((event.motion.x > buttons[B_EXIT].box.x 
                && event.motion.x <= buttons[B_EXIT].box.x + buttons[B_EXIT].box.w) 
                ...
                else buttons[B_LOAD].hover = false;
            break;
            case SDL_MOUSEBUTTONDOWN:
                if ((event.motion.x > buttons[B_EXIT].box.x 
                && event.motion.x <= buttons[B_EXIT].box.x + buttons[B_EXIT].box.w) 
...
bool createNewMap(Editor *e, Level l, const char *filename)
{
    ...
    if (fp == NULL) success = false;
    else 
    {
        for (int i = 0; i < l.total_tile_pieces; i++) e->fileBuffer[i] = 135;

        fwrite(e->fileBuffer, sizeof(e->fileBuffer[0]), l.total_tile_pieces, fp);
...
bool loadMap(tile set[], unsigned char buffer[], Level level, char str[]) 
{
    ...
    else 
    {
        ...
        for (int i = 0; i < level.total_tile_pieces; i++)
        {
            ...
            set[t].box.x = (((t % 20) + 1) << 5) - level.tile_size;  // weird bit hacking, is it faster though?
...
void editRender(Editor *editor, SDL_Renderer *renderer, texture sheet, FC_Font *font, Level level)
{
    // set the viewport camera
    setCamera(&editor->camera, editor->viewX, editor->viewY);
    editor->levelRect.x = editor->tileSet[0].box.x - editor->camera.x;  // set level rect
    ...
    renderShortcuts(renderer, &sheet, editor->hudShortcuts, editor->tilePieceClips);
    ...
    if (editor->save)   // maybe move somewhere else?
    {
        ...
        FC_DrawColor(
            font, 
            renderer, 
            (SCREEN_WIDTH >> 1) - ((sizeof str) * 16), // experimental
...
void loadRender(Editor *e, SDL_Renderer *renderer, FC_Font *texture, Button buttons[], SDL_Rect fileLoader, L_File list[])
{
    // file window
    SDL_SetRenderDrawColor(renderer, 0x80, 0x80, 0x80, 0xff);
...
void renderTiles(texture *sheet, Editor editor, Level level)  
{
    ...
    for (int i = 0; i < level.total_tiles; i++)
    {
        if (checkCollision(editor.tileSet[i].box, editor.camera, editor.zoom))
        {
            ...
            && editor.tileSet[i].set[3] == 135) continue;    // testing performance !!!
            ...
            if (editor.zoom)
            {
                ...
                x2 = (editor.tileSet[i].box.x + level.tile_piece_size << 1) - editor.camera.x;
                y2 = (editor.tileSet[i].box.y + level.tile_piece_size << 1) - editor.camera.y;
            }
            else 
            {
                ...
                x2 = editor.tileSet[i].box.x + level.tile_piece_size - editor.camera.x;
                y2 = editor.tileSet[i].box.y + level.tile_piece_size - editor.camera.y;
            }
...
void renderTilePieces(texture *sheet, SDL_Rect *clips, Level level)
{
    ...
    for (int i = 0; i < level.tile_pieces; i++)
    {
        ...
        renderTexture(
            sheet, 
            (((i % 17) + 1) << 4) - level.tile_piece_size,  // weird bit hacking, is it faster though?
...
void renderGrid(SDL_Renderer *r, SDL_Rect *c, short grid, bool zoom, Level level)
{
    if (grid)
    {
        int x = 0, 
        ...
        for (int i = 0; i < tiles; i++)
        {
            if (x == 0 && i) 
                SDL_RenderDrawLine(
                    ...
                    x - c->x, 
                    y - c->y, 
                    zoom ? level.width_z - c->x : level.width - c->x, 
                    y - c->y);
                ...
                SDL_RenderDrawLine(
                    ...
                    y - c->y, 
                    x - c->x, 
                    zoom ? level.height_z - c->y : level.height - c->y);

            if (zoom) x += (grid == 2) ? level.tile_piece_size_z : level.tile_size_z;
            else x += (grid == 2) ? level.tile_piece_size : level.tile_size;
...
void renderShortcuts(SDL_Renderer *r, texture *sheet, hudTile hudTile[], SDL_Rect *clip)
{
...
bool checkCollision(SDL_Rect a, SDL_Rect b, bool zoom)
{
    ...
    int leftA = zoom ? a.x << 1 : a.x, 
        leftB = b.x, 
        rightA = zoom ? a.x + a.w << 1 : a.x + a.w, 
        rightB = b.x + b.w, 
        topA = zoom ? a.y << 1: a.y, 
        topB = b.y, 
        bottomA = zoom ? a.y + a.h << 1 : a.y + a.h, 
        bottomB = b.y + b.h;
